...
*微信自定义表格与伪网格防止词条粘连*（**关键坑**）：
- 微信公众号的表格绝大多数使用自定义的 `<section>` / `<div>` 或无 `<tr>` 的伪网格组件渲染。
- `stripTags` 脱壳时，对 `</td>`, `</th>`, `</div>`, `</span>`, `</p>` 等单元格闭合标签必须**强制注入半角空格**，防止表头 `Keyword` 与数据 `Volume` 及数字 `1900` 粘在一起合并为惨不忍睹的乱码字符串 `KeywordVolume1900`。
...
4. **程序化硬核自动化断言 (Programmatic Automated Audit Assertions)**：生成 Markdown 后，必须自动运行程序断言脚本，扫描全文本：
   - 绝无任何单行奇数个/错位/未匹配闭合的 `**`；
   - 绝无任何 `&nbsp;` 实体字符与不可见字符（`\u00a0`, `\u200b`, `\ufeff`, ``）；
...
## 📌 产品待办清单与零成本架构 (TODO & Architecture Roadmap)

### 1. 浏览器插件端 (Chrome Extension - 场景一)
- [ ] **知乎问答悬浮球与控制浮窗**：当用户在知乎问答页面时，在右下角悬浮窗提供 `[ Top 5 回答 ▾ ]` 和 `[ 按热度/赞同数 ▾ ]` 选择器。
- [ ] **终极零成本架构**：所有抓取与 `@herdown/core` 转换直接运行在用户浏览器本地，利用用户已有登录 Cookie 完美绕过 `zh-zse-ck` 防爬盾，实现 $0 云端算力与带宽成本。

### 2. Web 网页端 (herdown.com - 场景二)
- [x] **知乎智能感应展开**：检测到 `zhihu.com/question/` 链接时平滑展开 `[ Top 5 / Top 10 / Top 20 高赞回答 ▾ ]` 下拉框，普通文章隐去不干扰。

### 3. 命令行 CLI (@herdown/cli - 场景三)
- [x] **知乎 Top N 参数支持**：默认无参数时零门槛抓取前 5 条高赞回答，极客用户可通过 `herdown <url> --limit 10` 指定条数。
